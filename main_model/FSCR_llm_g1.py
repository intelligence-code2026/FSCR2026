'''本程序采用第一组大模型（qwen-turbo、deepseek-chat）进行细粒度类案检索，包括生成合成案例，优化提示词和相似度阈值，
用测试集计算Precision、Recall、F1-score，进行交叉验证、消融实验、生成裁判规则。'''
'''This program utilizes the first group of large language models (Qwen-Turbo, DeepSeek-Chat) to perform 
...
file_path = '.\\data\\query'+str(num_query)+'.txt' 
...
doc.save(rf'.\data\output\result{num_query}.docx')
