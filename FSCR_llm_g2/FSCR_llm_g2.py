'''本程序采用第一组大模型（ernie-4.5-turbo-128、kimi-latest）进行细粒度类案检索，包括生成合成案例，优化提示词和相似度阈值，
用测试集计算Precision、Recall、F1-score，进行交叉验证、消融实验、生成裁判规则。'''
'''This program utilizes the first group of large language models (ernie-4.5-turbo-128、kimi-latest) to perform 
FSCR. Key functionalities include generating synthetic cases, optimizing prompts and similarity thresholds, 
calculating Precision, Recall, and F1-score on the test set, conducting cross-validation and ablation studies, 
and generating adjudication rules.'''
...
try:             
    ...
    # 读取候选案例文本
    xlsx_candidation = openpyxl.load_workbook(r'.\data\candidate_case.xlsx')
...
# Create a new PipelineModule, registering only the components that need optimization as dspy.Module attributes
...
# Instantiate the TargetedPipelineModule

pipeline_module_targeted = TargetedPipelineModule(summarizer=summarizer)

# 优化前评估（使用默认阈值）
# Pre-optimization evaluation (using default threshold)
...
# The optimizer will only adjust the attributes declared as dspy.Module within the TargetModule
...
# After optimization, retrieve the optimized predictor instance.
...
# Open the file and write content (with auto-closing), then update the prompt
...
final_threshold={1:0.748927032,2:0.682200337,3:0.765677072,4:0.766117281,5:0.761023332}

print(f'验证集包含的数据共{len(val_set)}条。')
# =============================================================================
# 直接采用优化模型计算指标
# Compute metrics directly using the optimized model
# =============================================================================
...
def objective_optuna(trial):
    ...
    for example in val_set:  # 使用验证集评估！
        # === MODIFIED FOR OPTUNA ===
        # pred = optimized_pipeline(**example.inputs(), similar_threshold=similar_threshold)
        pred = pipeline_module_targeted(**example.inputs(), similar_threshold=similar_threshold)
        score = my_metric(example, pred)
        if score != -1:
            total_f1 += score
            valid_count += 1
...
print("\n开始使用 Optuna 优化 similar_threshold_value（基于未经优化的 pipeline）...")

study = optuna.create_study(direction='minimize')
study.optimize(objective_optuna, n_trials=30)

best_threshold_tom = study.best_params['similar_threshold_value']
best_f1 = -study.best_value

end_time_optuna=time.time()
cost_time_optuna=(end_time_optuna-start_time_optuna)/3600
print("\n Optuna 优化完成！")
print(f"最佳 similar_threshold_value: {best_threshold_tom:.4f}")
print(f"对应验证集平均 F1: {best_f1:.4f}")
...
final_threshold_tom[epoch_num_cv] = best_threshold_tom



#loaded_pipeline_1是优化前的原始模型
# loaded_pipeline_1_1 = copy.deepcopy(loaded_pipeline_1)
loaded_pipeline_1_1 =pipeline_module_targeted

scores_test_1_1 = [
    evalution_metric(example, loaded_pipeline_1_1(**example.inputs(), similar_threshold=final_threshold_tom[num_selected]))
    for example in testset
    ]
...
sheet1['F'+str(line)]=final_threshold_tom[num_selected]
sheet1['G'+str(line)]=len(testset) 
sheet1['H'+str(line)]=num_selected   
xlsx.save(r'.\\data\\result.xlsx')
xlsx.close()  # 确保文件被关闭
...
loaded_pipeline_3 = TargetedPipelineModule(summarizer=summarizer_candidate)
loaded_pipeline_3.load(".\\data\\dspy_opted_model\\opted_model\\optimized_dspy_False"+str(num_selected)+'.pkl')      
...
dict_pre=loaded_pipeline_3(**query_example.inputs(),similar_threshold=final_threshold[num_selected])
...
if len(query2candidate)!=0:   
    ...
    doc.add_paragraph('本次没有查询到相关案例，可尝试更换模型后重试。')
